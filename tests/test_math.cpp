...
void test_fixed_arithmetic() {
    ...
    auto sum = a + b;
    if (sum.to_int() != 7) { std::fprintf(stderr, "  ✗ addition\n"); return; }

    auto prod = a * b;
    if (prod.to_int() != 12) { std::fprintf(stderr, "  ✗ multiplication\n"); return; }

    auto div = prod / b;
    if (div.to_int() != 3) { std::fprintf(stderr, "  ✗ division\n"); return; }
...
void test_sin_cos_determinism() {
    ...
    if (s1.raw != s2.raw) { std::fprintf(stderr, "  ✗ sin not deterministic\n"); return; }

    // sin(0) == 0, sin(90) == 1, cos(0) == 1
    if (df::sin_deg(0).to_int() != 0) { std::fprintf(stderr, "  ✗ sin(0)\n"); return; }
    if (df::sin_deg(90).to_int() != 1) { std::fprintf(stderr, "  ✗ sin(90)\n"); return; }
    if (df::cos_deg(0).to_int() != 1) { std::fprintf(stderr, "  ✗ cos(0)\n"); return; }

    std::fprintf(stderr, "  ✓ sin/cos determinism and values\n");
}

void test_clamp() {
    if (df::clamp(5, 0, 10) != 5) { std::fprintf(stderr, "  ✗ clamp mid\n"); return; }
    if (df::clamp(-1, 0, 10) != 0) { std::fprintf(stderr, "  ✗ clamp lo\n"); return; }
    if (df::clamp(15, 0, 10) != 10) { std::fprintf(stderr, "  ✗ clamp hi\n"); return; }
